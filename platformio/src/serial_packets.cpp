...
void SerialPacketsClient::loop() {
  ...
  while (n > 0) {
    ...
    for (int i = 0; i < bytes_read; i++) {
      PacketDecoder::DecoderStatus status =
          _packet_decoder.decode_next_byte(rx_transfer_buffer[i]);
      // if (status != PacketDecoder::IN_PACKET) {
        _logger.log("%02x -> %d %hu", rx_transfer_buffer[i], status, _packet_decoder.len());
      // }
      if (status == PacketDecoder::PACKET_DECODED) {
        ...
        switch (packet_type) {
          ...
          case TYPE_MESSAGE:
            ...
            break;
          default:
            _logger.log("Unknown incoming packet type: %02hhx", packet_type);
        }
      }
    }
  }
...
void SerialPacketsClient::process_decoded_command_packet(
  ...
  if (!_command_handler) {
    _logger.log("ERROR: No handler for incoming command");
    return;
  }
  ...
  if (!_packet_encoder.encode_response_packet(
          metadata.cmd_id, status, _tmp_data1, _tmp_data2, insert_pre_flag)) {
    _logger.log("ERROR, failed to encode response packet. Dropping.");
    return;
  }
  ...
  if (_data_stream->availableForWrite() < size) {
    _logger.log(
        "ERROR: Insufficient TX buffer space for sending a response packet "
        "(%hu).",
        size);
    return;
  }
  ...
  if (written < size) {
    force_next_pre_flag();
    _logger.log("ERROR, only %hu of %hu of response packet bytes were written",
                written, size);
  }
}
...
void SerialPacketsClient::process_decoded_response_packet(
  ...
  if (!context) {
    _logger.log(
        "ERROR: incoming response packet has no pending command. May timeout.");
    return;
  }
  if (!context->response_handler) {
    _logger.log("ERROR: Pending command has a null response handler.");
    return;
  }
...
void SerialPacketsClient::process_decoded_message_packet(
  ...
  // }
  if (!_message_handler) {
    _logger.log("ERROR: No message handler to dispatch an incoming message.");
    return;
  }
...
bool SerialPacketsClient::sendCommand(
  ...
  if (!response_handler) {
    _logger.log("ERROR: Trying to send a command without a response handler.");
    return false;
  }
  ...
  if (timeout_millis < MIN_CMD_TIMEOUT_MILLIS ||
    ...
    _logger.log("ERROR: Invalid command timeout %hu, should be in [%d, %d]",
  ...
  if (!cmd_context) {
    _logger.log("ERROR: Can't send command, too many commands in progress (%d)",
                MAX_CMD_CONTEXTS);
    return false;
  }
  ...
  if (!_packet_encoder.encode_command_packet(new_cmd_id, endpoint, data,
                                             _tmp_data1, insert_pre_flag)) {
    _logger.log("ERROR: Failed to encode command packet");
    return false;
  }
  ...
  _logger.log("Available: %d, size: %hu", available, size);
  if (available < size) {
    _logger.log(
        "ERROR: Can't send a command packet of size %hu, TX buffer has only %d "
        "bytes free",
        size, available);
    return false;
  }
  ...
  _logger.log("Written %hu of %hu command packet bytes", written, size);

  if (written < size) {
    _logger.log("ERROR, only %hu of %hu of a command packet bytes were written",
                written, size);
    force_next_pre_flag();
    return false;
  }
  ...
  cmd_id = new_cmd_id;
  _logger.log("Command packet written ok, cmd_id = %08x", cmd_id);
  return true;
}
...
bool SerialPacketsClient::sendMessage(byte endpoint,
  ...
  // Encode the packet in wire format.
  if (!_packet_encoder.encode_message_packet(endpoint, data, _tmp_data1,
                                             insert_pre_flag)) {
    return false;
  }
  ...
  if (written < size) {
    _logger.log("ERROR, only %hu of %hu of a message packet bytes were written",
                written, size);
    force_next_pre_flag();
  }
